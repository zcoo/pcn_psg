...
import time
from data_util_my import lmdb_dataflow, get_queued_data
...
def train(args):
    with tf.device('/gpu:0'):
        ...
        images_pl = tf.placeholder(tf.float32, (args.batch_size, 224, 224, 3), 'images')
        ...
        model = model_module.Model(images_pl, inputs_pl, npts_pl, gt_pl, alpha, args.batch_size,mode=args.mode)
        ...
        for step in range(init_step + 1, args.max_step + 1):
            ...
            ids, images, inputs, npts, gt = next(train_gen)
            start = time.time()
            feed_dict = {images_pl: images, inputs_pl: inputs, npts_pl: npts, gt_pl: gt, is_training_pl: True}
            ...
            if step % args.steps_per_eval == 0:
                ...
                for i in range(num_eval_steps):
                    start = time.time()
                    ids, images, inputs, npts, gt = next(valid_gen)
                    feed_dict = {images_pl: images, inputs_pl: inputs, npts_pl: npts, gt_pl: gt, is_training_pl: False}
                ...
                if step % args.steps_per_visu == 0:
                    ...
                    for i in range(0, args.batch_size, args.visu_freq):
                        ...
                        with open(plot_path, "wb") as f:
                            pickle.dump([pcds, model.visualize_titles], f)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lmdb_train', default='/home/zc/data/data/02958343_png_train.lmdb')
    parser.add_argument('--lmdb_valid', default='/home/zc/data/data/02958343_png_test.lmdb')

    # 运行的时候修改这两行
    parser.add_argument('--log_dir', default='log/pcn_cd_pcnpsg2')
    parser.add_argument('--mode', default=2)
    parser.add_argument('--model_type', default='pcn_my')
