...
import time
from io_util import read_pcd, save_pcd, load_image
from tf_util import chamfer, earth_mover
from visu_util import plot_pcd_three_views
from data_util_my import resample_pcd


# Test my model on cd and emd loss
...
def test(args):
    with tf.device('/gpu:0'):
        ...
        model = model_module.Model(images, inputs, npts, gt, tf.constant(1.0), 1, mode=args.mode)
        ...
        for i, model_id in enumerate(model_list):
            image = load_image(os.path.join(args.data_dir, 'image', '%s.png' % model_id.split('/')[1]))
            partial = read_pcd(os.path.join(args.data_dir, 'partial', '%s.pcd' % model_id.split('/')[1]))
            complete = read_pcd(os.path.join(args.data_dir, 'complete', '%s.pcd' % model_id.split('/')[1]))
            complete = resample_pcd(complete, 16384)

            start = time.time()
            completion = sess.run(model.outputs, feed_dict={images: image, inputs: [partial], npts: [partial.shape[0]]})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--list_path', default='/home/zc/data/data/02958343_test.txt')
    parser.add_argument('--data_dir', default='/home/zc/data/data/test_02958343')
    parser.add_argument('--model_type', default='pcn_emd_my')
    parser.add_argument('--checkpoint', default='/home/zc/pcn_psg/log/pcn_emd_pcnpsg2/model-300000')
    parser.add_argument('--results_dir', default='/home/zc//exp_result/pcn_emd_pcnpsg2')
