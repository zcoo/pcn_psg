...
class BatchData(dataflow.ProxyDataFlow):
    ...
    def _aggregate_batch(self, data_holder, use_list=False):
        ...
        images = np.stack(x[1] for x in data_holder)
        inputs = [resample_pcd(x[2], self.input_size) if x[2].shape[0] > self.input_size else x[2]
            for x in data_holder]
        inputs = np.expand_dims(np.concatenate([x for x in inputs]), 0).astype(np.float32)
        npts = np.stack([x[2].shape[0] if x[2].shape[0] < self.input_size else self.input_size
            for x in data_holder]).astype(np.int32)
        gts = np.stack([resample_pcd(x[3], self.gt_size) for x in data_holder]).astype(np.float32)
        #print(ids.shape,images.shape,inputs.shape,npts.shape,gts.shape)
        return ids, images, inputs, npts, gts
